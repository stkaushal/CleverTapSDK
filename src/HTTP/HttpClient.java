	...
	public JSONObject postRequest(String url, JSONObject obj) throws IOException, InterruptedException
	{
		...
		while (true) {
			...
			try {
				...
				{
					System.out.println("Server status " + httpResponse.getStatusLine().getStatusCode() + ". Retrying to post in " + timeToRetry*2 +  " seconds...");
					TimeUnit.SECONDS.sleep(timeToRetry*2);
					timeToRetry = 2*timeToRetry;
			        if(tryCount == 0)
			        {
				...
				System.out.println("Failed! Timeout in post request, retrying in " + timeToRetry*2 + " seconds...");
				TimeUnit.SECONDS.sleep(timeToRetry*2);
				timeToRetry = 2*timeToRetry;
			} catch(HttpHostConnectException connectionException) {
				System.out.println("Connection refused by server during post request. Retrying in " + timeToRetry*2 + " seconds...");
				TimeUnit.SECONDS.sleep(timeToRetry*2);
				timeToRetry = timeToRetry*2;
			} catch (IOException ioException) {
				System.out.println("IOException caught during post request: " + ioException + "\n Retrying in " + timeToRetry*2 + " seconds...");
				TimeUnit.SECONDS.sleep(timeToRetry*2);
				timeToRetry = timeToRetry*2;
			}
			finally {
	...
	public JSONObject getRequest(String url) throws IOException, InterruptedException {
		...
		while (true) {
			...
			try {
				...
				if(httpResponse.getStatusLine().getStatusCode()>=500 && httpResponse.getStatusLine().getStatusCode()<=599)
				{
					System.out.println("Server status " + httpResponse.getStatusLine().getStatusCode() + ". Retrying to get in " + timeToRetry*2 +  " seconds...");
					TimeUnit.SECONDS.sleep(timeToRetry*2);
					timeToRetry = 2*timeToRetry;
			        if(tryCount == 0)
			        {
				...
				System.out.println("Failed! Timeout in get request, retrying in " + timeToRetry*2 + " seconds...");
				TimeUnit.SECONDS.sleep(timeToRetry*2);
				timeToRetry = 2*timeToRetry;
			} catch(HttpHostConnectException connectionException) {
				System.out.println("Connection refused by server during get request. Retrying in " + timeToRetry*2 + " seconds...");
				TimeUnit.SECONDS.sleep(timeToRetry*2);
				timeToRetry = timeToRetry*2;
			} catch (IOException ioException) {
